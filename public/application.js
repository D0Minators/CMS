...
var config = {
  apiOrigins: {
    production: 'https://sserpdrow-api.herokuapp.com',
...
var updateContent = function updateContent(data, id) {
  return $.ajax({
...
var getAllContent = function getAllContent() {
  return $.ajax({
    url: config.apiOrigin + '/contents',
    method: 'GET'
  });
...
var getOneBlog = function getOneBlog(email) {
...
var getOnePage = function getOnePage(email) {
...
var contentEvents = __webpack_require__(18);
...
var signInFailure = function signInFailure() {
  $('#message').text('Error with your login, buddy!').then($('#change-password').hide()).then($('#sign-out').hide());
};
...
var signOutFailure = function signOutFailure() {
  $('#message').text('Error with signing out, buddy!').then($('#change-password').hide()).then($('#sign-out').hide());
};
...
var onCreateContent = function onCreateContent(event) {
  var data = getFormFields(this);
  event.preventDefault();
  api.createContent(data).then(ui.createContentSuccess).catch(ui.createContentFailure);
};

var onGetPostContent = function onGetPostContent(event) {
  // only fetch data when accordion is going to expand
  if ($(event.target).attr('class') === 'collapsed') {
    api.getContent().then(ui.getPostsSuccess).catch(ui.getPostsFailure);
  }
};

var onGetPageContent = function onGetPageContent(event) {
  // only fetch data when accordion is going to expand
  if ($(event.target).attr('class') === 'collapsed') {
    api.getContent().then(ui.getPagesSuccess).catch(ui.getPagesFailure);
  }
};

var onViewOneBlog = function onViewOneBlog(event) {
  var email = getFormFields(this).email;
  event.preventDefault();
  api.getOneBlog(email).then(ui.getOneBlogSuccess).catch(ui.getOneBlogFailure);
};

var onChangeCreateContentMessage = function onChangeCreateContentMessage(event) {
  $('#message').text('Start Telling Your Story:');
};

var onGetPageList = function onGetPageList(event) {
  event.preventDefault();
  var email = getFormFields(this).email;
  api.getOnePage(email).then(ui.populatePageList).catch(ui.getPageFailure);
};
...
module.exports = {
  ...
  onGetPostContent: onGetPostContent
};
/* WEBPACK VAR INJECTION */}.call(exports, __webpack_require__(0)))
...
var store = __webpack_require__(3);
var api = __webpack_require__(8);
var showBlogsTemplate = __webpack_require__(20);
var showContent = __webpack_require__(37);
var showPageTemplate = __webpack_require__(38);

var createContentSuccess = function createContentSuccess(data) {
...
var createContentFailure = function createContentFailure() {
  $('#message').text('Error on creating content. Please Check All Fields Are Filled In.');
};

// Displays blog entries for one user/owner
var getOneBlogSuccess = function getOneBlogSuccess(data) {
  $('.showblogs').addClass('hidden');
  $('.showblogs').empty();
  var matchingEntries = data.contents.filter(function (content) {
  ...
  if (matchingEntries.length === 0) {
    ...
    $('form').trigger('reset');
    var showBlogsHtml = showBlogsTemplate({ contents: matchingEntries });
    $('.showpage').empty();
    $('.showpage').addClass('hidden');
...
var selectedPage = function selectedPage(matchingEntries, value) {
  var pickPage = matchingEntries.filter(function (content) {
    return content._id === value;
  });
  var showPageHtml = showPageTemplate({ contents: pickPage });
  $('.showpage').empty();
  $('.showblogs').empty();
  $('.showblogs').addClass('hidden');
  $('#selectPage').addClass('hidden');
...
var getContentSuccess = function getContentSuccess(data) {
  $('#message').text('You have succesfully received content!');
};

var getContentFailure = function getContentFailure() {
  $('#message').text('Error on getting content.');
};

var getPostsSuccess = function getPostsSuccess(data) {
  $('.post-list').empty();
  var matchingPosts = data.contents.filter(function (content) {
    return content.type === 'post';
  });
  if (matchingPosts.length === 0) {
    ...
    var showContentHTML = showContent({
    ...
    $('.delete-content').on('click', function (event) {
      ...
      api.deleteContent(id).then(deletePostSuccess).catch(deletePostFailure);
    });
...
var onEditPost = function onEditPost(event) {
  ...
  var saveButtonLi = $(this).parent().siblings()[5];
  $(saveButtonLi).children().show();
  $('.edit-content').hide();
  var id = $(this).parent().parent().data('id');
  var title = $(this).parent().siblings()[0];
  var date = $(this).parent().siblings()[1];
  var text = $(this).parent().siblings()[2];
  var type = $(this).parent().siblings()[3];
...
var onSavePost = function onSavePost(id, title, date, text, type) {
  var newTitle = $(title).text();
  var newText = $(text).text();
  var newDate = $(date).text().trim();
  ...
  title.remove();
  text.remove();
  api.updateContent(data, id).then(updatePostSuccess).then($('.edit-content').show()).catch(updatePostFailure);
};
...
var getPagesSuccess = function getPagesSuccess(data) {
  $('.page-list').empty();
  var matchingPages = data.contents.filter(function (content) {
    return content.type === 'page';
  });
  if (matchingPages.length === 0) {
    ...
    $('.delete-content').on('click', function (event) {
      ...
      $(this).parent().parent().remove();
      var id = $(this).parent().parent().data('id');
      api.deleteContent(id).then(deletePageSuccess).catch(deletePageFailure);
    });
    $('.edit-content').on('click', onEditPage);
  }
};

var onEditPage = function onEditPage(event) {
  event.preventDefault();
  var saveButtonLi = $(this).parent().siblings()[5];
  $(saveButtonLi).children().show();
  $('.edit-content').hide();
...
var onSavePage = function onSavePage(id, title, date, text, type) {
  var newTitle = $(title).text();
  var newText = $(text).text();
  var newDate = $(date).text().trim();
  var newType = $(type).text();
  ...
  title.remove();
  text.remove();
  api.updateContent(data, id).then(updatePageSuccess).then($('.edit-content').show()).catch(updatePageFailure);
};
...
var updatePostSuccess = function updatePostSuccess() {
  ...
  api.getContent().then(getPostsSuccess).catch(getPostsFailure);
};
...
var updatePageSuccess = function updatePageSuccess() {
  $('#message').text('Your Page Has Been Updated');
  api.getContent().then(getPagesSuccess).catch(getPagesFailure);
};

var updatePageFailure = function updatePageFailure() {
  $('#message').text('Error on updating page');
};
module.exports = {
  createContentSuccess: createContentSuccess,
  createContentFailure: createContentFailure,
  ...
  getContentSuccess: getContentSuccess,
  getContentFailure: getContentFailure,
  getOneBlogSuccess: getOneBlogSuccess,
  getOneBlogFailure: getOneBlogFailure,
...
module.exports = (Handlebars["default"] || Handlebars).template({"1":function(container,depth0,helpers,partials,data,blockParams) {
  ...
  return "\n<div class=\"blog-header\">\n  <div class=\"container\">\n    <h1 class=\"blog-title\">Blog</h1>\n  </div>\n</div>\n\n<div class=\"row\">\n  <div class=\"col-sm-8 blog-main\">\n"
    + ((stack1 = helpers.each.call(depth0 != null ? depth0 : (container.nullContext || {}),(depth0 != null ? depth0.contents : depth0),{"name":"each","hash":{},"fn":container.program(1, data, 1, blockParams),"inverse":container.noop,"data":data,"blockParams":blockParams})) != null ? stack1 : "")
    + "  </div>\n</div>\n";
...
module.exports = (Handlebars["default"] || Handlebars).template({"1":function(container,depth0,helpers,partials,data,blockParams) {
  ...
  return "  <ul data-id=\""
    ...
    + " </li>\n    <li>"
    + alias2(alias1(((stack1 = blockParams[1][0]) != null ? stack1.text : stack1), depth0))
    + "</li>\n    <li>"
    + alias2(alias1(((stack1 = blockParams[1][0]) != null ? stack1.type : stack1), depth0))
    + "</li>\n    <li><button class=\"btn btn-info btn-sm delete-content\">Delete Content</button></li>\n    <li><button class= \"edit-content btn btn-info btn-sm\">Edit Content</button></li>\n    <li><button class= \"save-content btn btn-info btn-sm\">Save Changes</button></li>\n  </ul>\n";
},"compiler":[7,">= 4.0.0"],"main":function(container,depth0,helpers,partials,data,blockParams) {
    var stack1;

  return ((stack1 = helpers.each.call(depth0 != null ? depth0 : (container.nullContext || {}),(depth0 != null ? depth0.contents : depth0),{"name":"each","hash":{},"fn":container.program(1, data, 1, blockParams),"inverse":container.noop,"data":data,"blockParams":blockParams})) != null ? stack1 : "");
},"useData":true,"useBlockParams":true});
